...
### Integration API
The Integration API works as expected when running in swarm mode although the environment variable `REPLICATED_INTEGRATIONAPI` is not automatically injected into your running container as it is with the Replicated Native scheduler. The [`PremkitAPIAddress`](/packaging-an-application/template-functions#premkitapiaddress) template function has been made available to you to inject the address into the container's environment.
